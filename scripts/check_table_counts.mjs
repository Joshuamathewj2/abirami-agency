...
function fetchCount(table) {
  return new Promise((resolve) => {
    const req = https.request({
      ...
      res.on('end', () => {
        resolve({ table, status: res.statusCode, contentRange: range, body });
      });
    });
...
async function run() {
  ...
  const resOrders = await fetchCount('orders');
  console.log('orders table count response:', resOrders.contentRange || resOrders.body);

  const resInquiries = await fetchCount('inquiries');
  console.log('inquiries table count response:', resInquiries.contentRange || resInquiries.body);
}
