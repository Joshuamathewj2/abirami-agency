...
export function StardustButton({
  ...
  const getStyles = () => {
    if (isDarkMode) {
      ...
      // Light Mode styles (optimized for Parryware theme)
      switch (variant) {
        case "aurora":
          return {
        ...
        case "glass":
          return {
            bg: "bg-white",
            border: "border-gray-100 hover:border-sky-300/40",
            textColor: "text-slate-800 hover:text-sky-600",
            glow: "shadow-[0_8px_30px_rgb(0,9,120,0.03)]",
          };
        case "cosmic":
        default:
          return {
            bg: "bg-sky-50/70 backdrop-blur-md",
            border: "border-sky-200/50 hover:border-sky-300/80",
            textColor: "text-sky-700 hover:text-sky-950",
            glow: "shadow-[0_0_15px_rgba(0,145,255,0.1)]",
          };
      }
    }
  };
  ...
  const classNameStr = `
    ...
    border border-solid ${styleObj.border} ${styleObj.bg} ${styleObj.textColor}
    ${sizeClasses[size] || ""} ${isHovered ? styleObj.glow : "shadow-sm"}
    ${className}
  `;
