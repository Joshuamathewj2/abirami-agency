...
export default function WhatsAppCenterPage() {
    ...
    if (res.data) {
      const formattedRequests: WhatsAppRequest[] = res.data.map((inq: any, index: number) => {
        ...
        return {
          ...
          customer: inq.customer_name || 'Unknown Customer',
          ...
          totalAmount: inq.total_amount || 0,
          items: (inq.inquiry_items || []).map((item: any) => {
          let image = 'https://placehold.co/400x300?text=No+Image';
          let finalSizeLabel = item.variant?.size_name || 'N/A';
          const dimParts = [];
          ...
          // Try to parse color from notes for legacy "Color Option X"
          if (inq.notes && (item.variant?.mattress?.name || '').toLowerCase().includes('bed')) {
            const match = inq.notes.match(/\(Color Option (\d+)\)/);
            if (match) {
              const idx = parseInt(match[1]);
              finalSizeLabel = `${finalSizeLabel} (${getColorName(idx)})`;
              if (sorted[idx - 1]) image = sorted[idx - 1].image_url;
            } else {
              // Try to find if it already has the mapped color name
              for (const [key, name] of Object.entries(BEDSPREAD_COLORS)) {
                if (inq.notes.includes(`(${name})`)) {
                   finalSizeLabel = `${finalSizeLabel} (${name})`;
          ...
          return {
              id: item.id,
              product: item.variant?.mattress?.name || 'Unknown Product',
              variant: 'Standard',
              size: finalSizeLabel,
              qty: item.quantity || 1,
  ...
  const sortedRequests = [...filteredRequests].sort((a, b) => {
    ...
    if (sortBy === 'status-pending') {
      const statusOrder = { 'Pending': 0, 'Contacted': 1, 'Completed': 2 };
  ...
  const generateWhatsAppMessage = (req: WhatsAppRequest) => {
    let msg = `*Order Request — Abirami Agency*\n`;
    msg += `👤 ${req.customer}\n`;
    msg += `📞 ${req.phone}\n`;
    msg += `📍 ${req.address}\n`;
    msg += `\n*Items:*\n`;
    ...
    if (req.discountAmount > 0) {
      msg += `\nSubtotal: ₹${calculateTotal(req.items).toLocaleString('en-IN')}`;
      msg += `\nDiscount Applied${req.couponId ? ` (${req.couponId})` : ''}: -₹${req.discountAmount.toLocaleString('en-IN')}`;
    }
  ...
  const updateStatus = async (id: string, newStatus: RequestStatus) => {
    ...
    // Update securely via Server Action
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="bg-emerald-50/50 border border-emerald-100 rounded-2xl p-5 shadow-sm text-center">
          <p className="text-xs font-bold text-emerald-700 uppercase tracking-widest mb-1">Completed</p>
      ...
      <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm">
        <div className="px-6 py-4 border-b border-gray-100 flex flex-col md:flex-row md:items-center justify-between gap-4 bg-gray-50/50">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-bold text-gray-900">Customer Requests</h3>
            <span className="text-xs font-medium text-gray-400 ml-2">₹0 revenue — status updates only</span>
          </div>
        ...
        <div className="overflow-auto max-h-[calc(100vh-250px)]">
          <table className="w-full text-left text-sm whitespace-nowrap min-w-[900px]">
            <thead className="bg-white border-b border-gray-100 text-xs font-black text-gray-400 uppercase tracking-widest sticky top-0 z-10">
              <tr>
                <th className="px-6 py-4">ORD ID</th>
                <th className="px-6 py-4">Customer</th>
                <th className="px-6 py-4">Phone</th>
                <th className="px-6 py-4 text-center hidden md:table-cell">Address</th>
                ...
                return (
                  <React.Fragment key={req.id}>
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-gray-500 truncate max-w-[150px] text-xs text-center hidden md:table-cell" title={req.address}>{req.address}</td>
                        ...
                        <td colSpan={9} className="p-0 border-b border-gray-200">
                          <div className="bg-[#fcfdfd] px-6 py-6 border-l-4 border-gray-900 shadow-inner">
                            ...
                            <div className="bg-white border border-gray-100 rounded-xl p-4 mb-6 flex flex-wrap gap-x-8 gap-y-2 text-sm">
                              <div><span className="text-gray-500 font-medium">Name:</span> <span className="font-bold text-gray-900">{req.customer}</span></div>
                              <div><span className="text-gray-500 font-medium">Phone:</span> <span className="font-bold text-gray-900">{req.phone}</span></div>
                              <div><span className="text-gray-500 font-medium">Address:</span> <span className="font-bold text-gray-900">{req.address}</span></div>
                            </div>
                              ...
                              <table className="w-full text-sm min-w-[600px]">
                                <thead className="bg-[#f8f9fa] text-xs font-black text-gray-400 uppercase tracking-widest border-b border-gray-100">
                                  <tr>
                                    ...
                                    <th className="px-4 py-3 text-left">Size / Weight</th>
                                ...
                                <tbody className="divide-y divide-gray-50">
                                  {req.items.map((item, idx) => (
                                    <tr key={idx}>
                                      ...
                                      <td className="px-4 py-3 text-gray-500">{item.variant}</td>
                                      ...
                                      <td className="px-4 py-3 text-right font-bold text-gray-900">₹{item.qty * item.unitPrice}</td>
                                    </tr>
                                  ))}
                                </tbody>
                                <tfoot className="bg-[#f8f9fa] border-t border-gray-100">
                                  {req.discountAmount > 0 && (
                                    <tr>
                                      <td colSpan={5} className="px-4 py-2 text-right text-xs font-black text-emerald-600 uppercase tracking-widest">
                                  ...
                                  <tr>
                                    <td colSpan={5} className="px-4 py-4 text-right text-xs font-black text-gray-400 uppercase tracking-widest">
                                      Grand Total
                                    </td>
